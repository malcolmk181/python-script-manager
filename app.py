...
def get_pyenv_python_path(python_version):
    ...
    try:
        ...
        raise FileNotFoundError("pyenv is not installed or not in PATH.")
    except Exception as e:
        raise RuntimeError(f"Failed to locate pyenv-managed Python3 {python_version}:\n{e}")
...
def setup_venv(script_name, python_version="3.13.0"):
    ...
    try:
        ...
        subprocess.run([os.path.join(env_path, "bin", "pip"), "install", "-r", requirements_file], check=True)
        messagebox.showinfo("Success", f"Virtual environment for {script_name} created successfully!")
    except subprocess.CalledProcessError as e:
        messagebox.showerror("Error", f"Failed to set up virtual environment for {script_name}:\n{e}")
...
def rebuild_venv(script_name):
    ...
    python_version = simpledialog.askstring(
        "Python Version",
        f"Enter the Python version to use for rebuilding {script_name} (e.g., 3.13.0):"
    )
    ...
    try:
        ...
        messagebox.showinfo("Success", f"Virtual environment for {script_name} has been rebuilt successfully.")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to rebuild virtual environment for {script_name}:\n{e}")
...
def update_script_list():
    ...
    for script in scripts:
        ...
        # Run Script Button
        btn_run = tk.Button(frame, text=f"Run {script}", command=lambda s=script: run_script(s))
        btn_run.pack(side="left", padx=5)

        # Rebuild Environment Button
        btn_rebuild = tk.Button(frame, text="Rebuild Env", command=lambda s=script: rebuild_venv(s))
...
def run_script(script_name):
    ...
    system = platform.system()
    if system == "Windows":
        subprocess.Popen(["start", "cmd", "/k", f"echo Running script {script_name} && echo. && echo. && {os.path.join(env_path, 'Scripts', 'python')} {script_path}"], shell=True)
    elif system == "Darwin":  # macOS
        ...
        subprocess.Popen(["x-terminal-emulator", "-e", f"echo 'Running script {script_name}' && echo && echo && {os.path.join(env_path, 'bin', 'python')} {script_path}"])
    else:
...
def add_script():
    ...
    python_version = simpledialog.askstring("Python Version", "Enter the Python version to use (e.g., python3, python3.9):")
    if not python_version:
        messagebox.showerror("Error", "Python version is required!")
        os.rmdir(script_dir)
        return
    ...
    def save_inputs():
        ...
        with open(os.path.join(script_dir, "main.py"), "w") as script_file:
            script_file.write(script_content)
        
        with open(os.path.join(script_dir, "requirements.txt"), "w") as req_file:
    ...
    # Requirements Input
    tk.Label(content_window, text="Requirements (requirements.txt):").pack(anchor="w", padx=5, pady=5)
